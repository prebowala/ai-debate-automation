...
def generate_turn(side, topic, round_num, turn_num, previous_exchange, model):
    ...
    if round_num == 1 and turn_num == 1:
        instruction = "This is the opening exchange. Establish a strong foundation without trying to deliver the entire debate at once."
    else:
        instruction = f"This is turn {turn_num} of round {round_num}. Respond directly to the immediately preceding argument. Do not restart the debate. Do not introduce yourself. Do not explain your task. Do not say 'in this round'. Do not recycle previous arguments. Add a genuinely useful new point."
...
"""
    response = query_openrouter(prompt, model, max_tokens=430, temperature=0.78)
    if response:
        return response
