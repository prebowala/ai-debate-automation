...
# FREE TIER OPTIMIZATION - reduce OpenRouter calls
FREE_MODE = True  # Set True if only using free OpenRouter models - reduces API calls by ~60%
if FREE_MODE:
    MAX_JUDGES = 3  # was 7, now 3 to save 4 calls per round
    MAX_VISUALS_PER_SEGMENT = 1  # was 2, now 1 visual LLM call per segment
    # Disable visual LLM calls entirely if you want zero extra calls - set to 0
    # MAX_VISUALS_PER_SEGMENT = 0  # uncomment to disable all visual planning calls
    TURNS_PER_SIDE_PER_ROUND = 3  # was 4, saves 6 calls total
    # Panel commentary 1 per round instead of 2
...
def plan_visuals(text, model):
    # FREE MODE: Skip LLM call for visuals if MAX_VISUALS is 0, or use cheap heuristic
    if FREE_MODE and MAX_VISUALS_PER_SEGMENT == 0:
        return []
    if FREE_MODE:
        # In free mode, use simple keyword extraction without LLM to save calls
        # Only call LLM if text contains strong visual nouns
        visual_keywords = ["adam", "eve", "garden", "eden", "serpent", "apple", "tree", "ark", "moses", "jesus", "cross", "heaven", "hell"]
        if not any(k in text.lower() for k in visual_keywords):
            return []
...
def render_video_segment(background=None, ui=None, audio=None, subtitles=None, output=None, position=None, glow_color=None, card_x=None, card_y=None, visual_plan=None, bg_path=None, ui_path=None, audio_path=None, subs_path=None, output_path=None, cx=None, cy=None, **kwargs):
    # Compatibility layer for both calling conventions (positional and keyword bg_path etc)
    # New code uses background, ui, audio, subtitles, output, position, glow_color, card_x, card_y
    # Old code from GitHub Actions uses bg_path, ui_path, audio_path, subs_path, output_path, cx, cy
    if background is None and bg_path is not None:
        background = bg_path
    if ui is None and ui_path is not None:
        ui = ui_path
    if audio is None and audio_path is not None:
        audio = audio_path
    if subtitles is None and subs_path is not None:
        subtitles = subs_path
    if output is None and output_path is not None:
        output = output_path
    if card_x is None and cx is not None:
        card_x = cx
    if card_y is None and cy is not None:
        card_y = cy
    if visual_plan is None:
        visual_plan = kwargs.get('visual_plan') or kwargs.get('eplan') or []
    # Ensure required
    if not all([background, ui, audio, subtitles, output]):
        raise ValueError(f"Missing required files for render_video_segment: bg={background}, ui={ui}, audio={audio}, subs={subtitles}, out={output}")
    if position is None:
        position = kwargs.get('position', 'center')
    if glow_color is None:
        glow_color = kwargs.get('glow', kwargs.get('glow_color', '#FFD700'))
    if card_x is None:
        card_x = kwargs.get('card_x', kwargs.get('cx', 960))
    if card_y is None:
        card_y = kwargs.get('card_y', kwargs.get('cy', 900))
...
def run_debate_pipeline():
    ...
    for round_num in range(1, ROUNDS + 1):
        ...
        if results:
            # FREE MODE: 1 commentary per round instead of 2 to save calls
            num_comments = 1 if FREE_MODE else 2
            ...
            if num_comments == 1:
                # Pick overall winner
                winner_judge = random.choice(results)
                comment = generate_panel_commentary(winner_judge["model"], winner_judge["winner"], topic, round_num, apologist_full, skeptic_full, panel_comments)
                panel_comments.append(comment)
                add_segment(comment, "AI Judge", "AI JUDGE — " + winner_judge["provider"].upper(), "center", "#3399FF", judge_voice_index=0)
            else:
